export const Products = [
    {
        id: "1",
        name: "Smartphone Samsung Galaxy Note 10",
        image: "https://a-static.mlcdn.com.br/618x463/smartphone-samsung-note-10-lite-tela-infinita-6-7-octa-core-2-7-128gb-4gb-12mp-12mp-3mp-preto/semprevoce/4196/97370e45278e557cd09d1484bca9dad9.jpg",
        price: 2389,
        details: `Smartphone Samsung Note 10 Lite Tela Infinita 6.7 
            Octa-Core 2.7 128GB 4GB 12MP+12MP+3MP - Preto Junto da S Pen,
            esse smartphone da Samsung é a ferramenta ideal para qualquer
            momento! Seja um fotógrafo profissional, empresário ou para 
            quem gosta de tirar fotos da família, o Galaxy Note 10 Lite 
            conta com um conjunto de câmera tripla na traseira, permitindo
            cliques perfeitos e sem esforço. Toda criatividade na palma da 
            sua mão com a câmera de selfie de 32MP. Sua tela de 6,7" FHD+ 
            Super AMOLED oferece um display infinito para maior imersão ao 
            assistir filmes e séries. Com 128GB de armazenamento interno, 
            ele é perfeito para guardar todas suas fotos, músicas ou vídeos! 
            Este produto tem um processador Octa-Core e 6GB de memória RAM,
            para um nível elevado de potência e produtividade sempre ao seu 
            alcance. Fique sempre conectado com a tecnologia 4G em um aparelho 
            dual chip! Tenha bateria para o dia todo, além de contar com 
            carregamento rápido. Este celular tem proteção personalizada com 
            biometria na tela, deixando seus conteúdos e dados confidenciais 
            protegidos e com total segurança.`,
        count: 1
    },
    {
        id: "2",
        name: "Smartphone Apple XR",
        image: "https://www.casasbahia-imagens.com.br/Control/ArquivoExibir.aspx?IdArquivo=1114527881",
        price: 4000,
        details: `O novo iPhone XR da Apple é sem dúvida muito poderoso. 
            Ele conta com as melhores tecnologias de hardware e software, 
            que juntos trabalham para que o usuário tenha um smartphone 
            dinâmico e com os mais avançados recursos para que você possa 
            fazer muito mais com seu iPhone.`,
        count: 1,
    },
    {
        id: "3",
        name: "Casa de Boneca",
        image: "https://http2.mlstatic.com/casa-casinha-de-boneca-polly-mdf-36-moveis-parquinho-D_NQ_NP_631510-MLB42874232317_072020-F.webp",
        price: 500,
        details: `1 casinha d e boneca (montada):

        Produzida em MDF de 3 mm;
        Janelas na cor mdf crú(janelas abertas e fixas)
        Telhado na cor mdf crú
        Escadas na cor mdf crú
        Paredes na cor Rosa bebê`,
        count: 1,
    },
    {
        id: "4",
        name: "Caneta Bic Azul",
        image: "https://images-na.ssl-images-amazon.com/images/I/71GAns1DbfL._AC_SL1500_.jpg",
        price: 33.60,
        details: `Caneta Esferográfica 1.0mm, BIC, 835205, Azul, 50 Unidades`,
        count: 1,
    },
    {
        id: "5",
        name: "Máscara de tecido",
        image: "https://t-static.dafiti.com.br/ntA82Kr5PS2Mta2mw66MJLJlUoE=/fit-in/430x623/static.dafiti.com.br/p/summer-soul-kit-com-20-masc%c3%a1ras-dupla-prote%c3%a7%c3%a3o-lav%c3%a1vel-summer-soul%2c-regul%c3%a1vel-azul-marinho-4882-1485895-1-zoom.jpg",
        price: 250,
        details: `Kit com 20 Mascáras Dupla Proteção Lavável`,
        count: 1,
    },
];